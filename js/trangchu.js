window.onload = function () {
  ...
  var numBanner = 8; // Số lượng hình banner  //khai báo khi trong thư mục banner img có 8 hình
  ...
  if (filters.length) {
    ...
    var soLuong = window.innerWidth < 1200 ? 3 : 3; //màn hình nhỏ thì chỉ hiển thị 3 sản phẩm màn hình lớn thì hiển thị 3 sản phẩm
...
function timKiemTheoKhuyenMai(list, tenKhuyenMai, soluong) {
  ...
  for (var i = 0; i < list.length; i++) {
    if (list[i].promo && list[i].promo.name == tenKhuyenMai) {
      result.push(list[i]);
      count--;
      if (count <= 0) break;
    }
