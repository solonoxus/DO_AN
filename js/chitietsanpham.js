...
window.onload = function () {
  //window.onload = function() { ... } là một hàm được gọi khi trang web được load với thuộc tính window.onload của JavaScript.
  khoiTao(); // gọi hàm khởi tạo ở file classes.js để tạo ra các sản phẩm và thêm vào list_products (mảng chứa tất cả sản phẩm)

  // thêm tags (từ khóa) vào khung tìm kiếm
  var tags = ["Samsung", "iPhone", "Vivo", "Oppo"];
  for (var t of tags) addTags(t, "index.html?search=" + t, true);

  phanTich_URL_chiTietSanPham(); // gọi hàm này để phân tích url và hiển thị thông tin sản phẩm ở file chitietsanpham.html

  // autocomplete cho khung tim kiem
  autocomplete(document.getElementById("search-box"), list_products); // gọi hàm autocomplete ở file search.js để thêm gợi ý khi tìm kiếm sản phẩm

  // Thêm gợi ý sản phẩm
  sanPhamHienTai && suggestion(); // nếu có sản phẩm hiện tại thì mới thêm gợi ý sản phẩm (sản phẩm tương tự) ở file chitietsanpham.html
};
...
function phanTich_URL_chiTietSanPham() {
  ...
  // Đổi title
  document.title = nameProduct + " - Thế giới điện tử";
  ...
  // Cập nhật hình nhỏ
  for (var img of sanPhamHienTai.imgs) {
    addSmallImg(img);
  }
...
// add hình
function addSmallImg(img) { // thêm hình vào thư viện hình nhỏ ở phần chi tiết sản phẩm 
  var newDiv =
    `<div class='item'>
                        <a>
                            <img src=` +
